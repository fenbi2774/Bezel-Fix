...
### 项目简介
PojavOutlineFix 是一个专为低端设备优化的 Fabric 模组，旨在修复在使用 PojavLauncher 运行 Minecraft 1.21.11 时出现的**方块选择框**和 **F3 准心**渲染错误。除了核心修复功能外，它还提供了一系列增强视觉体验的可定制功能。
...
2. 下载最新版本的 PojavOutlineFix `.jar` 文件并放入 `mods` 文件夹。
3. **必要依赖**：
...
要从源代码构建 PojavOutlineFix 模组，请确保已克隆项目仓库并安装了 **JDK 21**。项目使用 Fabric Loom 和 Gradle 构建系统。
...
### Introduction
PojavOutlineFix is a Fabric mod optimized for low-end devices, specifically designed to fix **block selection box** and **F3 crosshair** rendering errors when running Minecraft 1.21.11 via PojavLauncher. Beyond core fixes, it provides various customizable features to enhance the visual experience.
...
2. Place the PojavOutlineFix `.jar` in your `mods` folder.
...
4. \*\*Recommended\*\*:
   ...
   To build a PojavOutlineFix mod from source, make sure you have cloned the project repository and installed JDK 21. The project uses Fabric Loom and Gradle to build the system
